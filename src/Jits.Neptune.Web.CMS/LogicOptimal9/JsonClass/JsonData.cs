    ...
    public static JObject ConvertMappingToOriginal(JObject data)
    {
        ...
        if (jsFieldOfTable != null && jsFieldOfTable.Count > 0)
        {
            ...
            foreach (var jsValue in data)
            {
                ...
                if (!string.IsNullOrEmpty(jsValue.Key) && jsValue.Key.Length > 0 && jsValue.Key.StartsWith("#"))
                {
                ...
                if (!string.IsNullOrEmpty(row))
                {
                    ...
                    {
                        r = (ConvertHashCodeToCount(char.ConvertToUtf32(row, 0)) + 1) * 36 + ConvertHashCodeToCount(char.ConvertToUtf32(row, 1));
                    }
                }

                if (!string.IsNullOrEmpty(col)) c = ConvertHashCodeToCount(char.ConvertToUtf32(col, 0));

                string keyMap = "";

                if (r >= 0 && c >= 0) keyMap = ((JArray)jsFieldOfTable[r])[c].ToString();
...
public class JsonDataMapping
{
    ...
    /// 
    /// </summary>
    public JsonDataMapping(JsonData clsJsonData)
    {
        if (clsJsonData != null)
        {
            T = clsJsonData.TABLENAME;
            if (clsJsonData.DATA is JObject)
            {
                D = ConvertOriginalToMapping((JObject)clsJsonData.DATA);
            }
            else
            {
                D = clsJsonData.DATA;
            ...
            //P = clsJsonData.FPK;
            if (clsJsonData.FPK is JObject)
            {
                P = ((JObject)clsJsonData.FPK).ConvertToJArray();
            }
            else
            {
                P = clsJsonData.FPK;
    ...
    /// 
    /// </summary>
    public static JObject ConvertOriginalToMapping(JObject data)
    {
        JArray jsFieldOfTable = O9Utils.GetFieldOfTable();
        int hC = 36;
        ...
        if (jsFieldOfTable != null && jsFieldOfTable.Count > 0)
        {
            ...
            foreach (var jsValue in data)
            {
                ...
                if (r >= hC)
                {
                    ...
                    row = char.ConvertFromUtf32(ConvertCountToHashCode(cv) - 1) + char.ConvertFromUtf32(ConvertCountToHashCode(r - cv * hC));
                }
                else
                {
                ...
                if (isFind)
                {
                    ...
                    {
                        mapping.Add("#" + row + char.ConvertFromUtf32(ConvertCountToHashCode(c)), jsValue.Value);
                    }
                    else
                    {
                        mapping.Add("#" + row + char.ConvertFromUtf32(ConvertCountToHashCode(c)), new JArray(jsValue.Value));
                    }
