...
public static class O9Utils
{
    ...
    /// 
    /// </summary>
    public static string GenJsonBodyRequest(object objJsonBody, string functionId, string sessionid = "",
    EnmCacheAction isResultCaching = EnmCacheAction.NoCached, EnmSendTypeOption sendtype = EnmSendTypeOption.Synchronize,
    string usrId = "-1", MsgPriority priority = MsgPriority.Normal)
    {
        try
        {
            ...
            string strRequest = objJsonBody != null ? JsonConvert.SerializeObject(objJsonBody) : string.Empty;

            //using var o9Client = new O9Client();
            var o9Client = new O9Client();
            string strResult = o9Client.SendString(strRequest, functionId, usrId, sessionid, isResultCaching, sendtype, priority);
            o9Client = null;
            return strResult;
        }
        catch (Exception ex)
    ...
    /// 
    /// </summary>
    public static string GenJsonFunctionRequest(UserSessions user, string ptxcode, JObject ptxbody, string ptxproc = "",
        string functionId = "", EnmCacheAction isCaching = EnmCacheAction.NoCached)
    {
        try
        {
            ...
            return GenJsonBodyRequest(new JsonFunctionMapping(clsJsonFunction), functionId, user.Ssesionid, isCaching,
    ...
    public static JsonFrontOffice FrontOffice(UserSessions userSessions, string txCode, JObject requestModel)
    {
    ...
    /// 
    /// </summary>
    public static string GenJsonFrontOfficeRequest(UserSessions user, string ptxcode, JObject ptxbody, 
        ...
        string pid = null, bool isMapping = true)
    {
        try
        {
            ...
            if (isMapping)
            {
                ...
                var result = GenJsonBodyRequest(frontOfficeMapping, functionId, user.Ssesionid, EnmCacheAction.NoCached, 
                    EnmSendTypeOption.Synchronize, user.Usrid.ToString());
                return result;
            }
                
            return GenJsonBodyRequest(clsJsonFrontOffice, functionId, user.Ssesionid, EnmCacheAction.NoCached,
                EnmSendTypeOption.Synchronize, user.Usrid.ToString());
        }
        catch (Exception ex)
        {
    ...
    public static JObject ProcessIfcFees(string txCode, JObject jsRequest)
    {
        if (jsRequest.ContainsKey("ifcfees") && !jsRequest["ifcfees"].IsNullOrEmpty())
        {
            ...
            if (rulefunc != null && !string.IsNullOrEmpty(rulefunc.VALUE))
            {
                ...
                {
                    paysrc = rule.ACTCDT.RemoveWhiteSpaceAndUpper().Replace("PAYSRC='", "").Replace("'", "").Trim();
                }
            }
    ...
    /// <returns></returns>
    public static async Task<JObject> GetInfoByQuery(string query, string relativeFields, bool isArray = false)
    {
    ...
    /// <returns></returns>
    public static async Task<JObject> GetCrossRate(string strFCCRCD, string strSCCRCD, string strFRT = "BK", string strSRT = "BK")
    {
        await Task.CompletedTask;
        ...
        try
        {
            ...
            {
                query = "SELECT ROUND(O9SYS.O9UTIL.GET_CROSS_RATE('@FCCRCD','@SCCRCD','@FRT','@SRT'),9) FROM DUAL".Replace("@FCCRCD", strFCCRCD).Replace("@SCCRCD", strSCCRCD)
                    .Replace("@FRT", strFRT).Replace("@SRT", strSRT);
                if (!query.Contains("@"))
                {
                    jsReturn = await GetInfoByQuery(query, "cross_rate");

                    if (jsReturn != null) return jsReturn;
                }
            }
        }
        catch (Exception ex)
    ...
    /// <param name="strSRT"></param>
    /// <returns></returns>
    public static async Task<JObject> GetBaseRate(string strCCRCD, string strRT = "BK", string strSRT = "BK")
    {
        await Task.CompletedTask;
        ...
        try
        {
            string query = string.Empty;
            JObject jsReturn = null;
            query = "SELECT ROUND(O9SYS.O9UTIL.GET_BCYRATE('@CCRCD', '@RT'),9) FROM DUAL".Replace("@CCRCD", strCCRCD).Replace("@RT", strRT);
            if (!query.Contains("@"))
            {
                jsReturn = await GetInfoByQuery(query, "bk_rate_currency");
                O9Utils.ConsoleWriteLine(jsReturn.ToString());
                if (jsReturn != null) return jsReturn;
            }
        }
        catch (Exception ex)
    ...
    {
        if (jsFo == null) return null;

        JObject js = JObject.FromObject(jsFo);

        JObject jsBody = (JObject)js.SelectToken("TXBODY");
        js.Remove("TXBODY");
        js.Add("txbody", jsBody.ToLowerKey());
        ...
        {
            // JArray jsFees = jsFo.IfcFeeToJArray(); 
    ...
    public static SearchFunc SearchFunc(object data, string sFunc)
    {
        ...
        var listKey = O9Client.memCached.GetValue(GlobalVariable.O9_GLOBAL_COMCODE + ".SEARCH." + sFunc.ToUpper());
    ...
    public static JsonBackOffice BackOffice(UserSessions userSessions, string txCode, string tableName, object model)
    {
        ...
        if (convert == null || convert.R != 0)
        {
            throw new Exception("Error BackOfficeRequest:" + (convert == null ? "Null data" : convert.M.ToString()));
        }

        return new JsonBackOffice(JsonConvert.DeserializeObject<JsonBackOfficeMapping>(convert.M.ToString()!));
    }

    /// <summary>
    /// 
    /// </summary>
    public static string GenJsonBackOfficeRequest(UserSessions userSessions, string pTXCODE, List<JsonData> pTXBODY,
        string functionId = "", EnmCacheAction isCaching = EnmCacheAction.NoCached,
        ...
        string pPRN = "")
    {
        try
        {
            ...
            // Sent to hosting
            var result = GenJsonBodyRequest(new JsonBackOfficeMapping(clsJsonBackOffice), 
                functionId,
    ...
    /// 
    /// </summary>
    public static JToken Search(string sql, int page = 0, string searchFunc = null)
    {
        ...
        try
        {
            ...
            string strResult = GenJsonBodyRequest(jsCondition, proc);
            if (!string.IsNullOrEmpty(strResult))
            {
                ...
                if (page == 1)
                {
                    ...
                    if(searchFunc != null)
                    {
                        ...
                        Singleton<PagingData>.Instance.Paging[searchFunc] = jPaging["total"].ToInt();
                    }
    ...
    /// 
    /// </summary>
    public static TransactionResponse AnalysisLoginResult(ILogger logger, string error, string wsName = "")
    {
    ...
    {
        if (strResponse.Equals("TIMEOUTAQ") || strResponse.Contains("COREERRORSYSTEM") ||
    ...
    /// 
    /// </summary>
    public static TransactionResponse AnalysisFrontOfficeResult(ILogger logger, string strResponse, bool ispare = true)
    {
        ...
        try
        {
            ...
            if (jsonResponse.IsOK())
            {
                if (ispare)
                {
                    clsJsonFrontOfficeMP =
                        JsonConvert.DeserializeObject<JsonFrontOfficeMapping>(jsonResponse.GetMessage());
                ...
                if (clsJsonFrontOffice.RESULT != null) TransactionResponse.SetResult(clsJsonFrontOffice.RESULT);
                if (clsJsonFrontOffice.IBRET != null) TransactionResponse.SetResult(clsJsonFrontOffice.IBRET);
            }
            else
            {
                ...
                else
                {
                    ...
                    {
                        TransactionResponse.SetCode(new CodeDescription(errorCode,
                            GetReasonString(logger, messageresult)));
                    }
                }
            }
    ...
    /// 
    /// </summary>
    public static TransactionResponse AnalysisFunctionResult(ILogger logger, string strResponse)
    {
        TransactionResponse TransactionResponse = new();
        ...
        try
        {
            ...
            if (jsonResponse.IsOK())
            {
                ...
                TransactionResponse.SetCode(Codetypes.Code_Success_Trans);
                if (clsFunction.RESULT != null) TransactionResponse.SetResult(clsFunction.RESULT);
            }
            else
            {
                string messageresult = jsonResponse.GetMessage();
                ...
                {
                    TransactionResponse.SetCode(new CodeDescription(errorCode, GetReasonString(logger, messageresult)));
                }
            }
    ...
    {
        logger.LogInformation("Core Return information: " + strResult);
    ...
    {
        if (String.IsNullOrEmpty(errName)) return "";
    ...
    public static string GetErrorByLang(JObject jsErrDescr)
    {
        ...
        {
            if (jsErrDescr != null) strErrDescr = GetStringFromJsonObjectByLang(jsErrDescr);
            if (String.IsNullOrEmpty(strErrDescr)) strErrDescr = GetStringFromJsonObjectByLang(jsErrDescr, "en");
        }
    ...
    public static string GetStringFromJsonObjectByLang(JObject jsObject, string lang = "")
    {
        try
        {
            if (jsObject != null)
            {
                if (String.IsNullOrEmpty(lang)) lang = GlobalVariable.O9_GLOBAL_LANG;
        ...
        try
        {
            ...
            if (clsJsonResponse != null)
            {
                if (clsJsonResponse.IsOK())
                {
                    ...
                    var jsonFrontOfficeMapping = JsonConvert.DeserializeObject<JsonFrontOfficeMapping>(message, JsonSerializerSetting);
    ...
    public static JObject AnalysisBackOfficeResult(string strResponse, JObject jsResult = null)
    {
        try
        {
            ...
            if (clsJsonResponse != null)
            {
                ...
                {
                    JsonBackOffice clsJsonBackOffice = new JsonBackOffice(
                        JsonConvert.DeserializeObject<JsonBackOfficeMapping>(clsJsonResponse.GetMessage(),
        ...
        {
            System.Console.WriteLine(ex.StackTrace);
            throw new Exception("AnalysisBackOfficeResult");
        }
    }
    ...
    /// 
    /// </summary>
    public static JObject AnalysisBOResult(string strResponse, JObject jsResult = null, bool isNeedMapping = true)
    {
        try
        {
            ...
            if (clsJsonResponse != null)
            {
                if (clsJsonResponse.IsOK())
                {
                    var backOfficeMapping = JsonConvert.DeserializeObject<JsonBackOfficeMapping>(clsJsonResponse.GetMessage(), JsonSerializerSetting);
                    JsonBackOffice clsJsonBackOffice = new JsonBackOffice(backOfficeMapping);
    ...
    /// <returns></returns>
    /// <exception cref="Exception"></exception>
    public static JObject AnalysisBOResultSingle(string strResponse, JObject jsResult = null, bool isNeedMapping = true)
    {
        try
        {
            JsonResponse clsJsonResponse = GetJsonResponse(strResponse);
            ...
            if (clsJsonResponse != null)
            {
                if (clsJsonResponse.IsOK())
                {
                    var dataMapping = JsonConvert.DeserializeObject<JsonDataMapping>(clsJsonResponse.GetMessage(), JsonSerializerSetting);
                    ...
                    {
                        //var rsOk = ((JObject)clsJsonBackOffice.TXBODY[0].DATA).ConvertToJObject();
                        jsResult = clsJsonBackOffice.TXBODY[0].BuildWorkflowResponseSuccess(isNeedMapping);
                    }
    ...
    public static DateTime? ConvertStringToDateTime(string strDateTime)
    {
        ...
        {
            CultureInfo culInfo = new CultureInfo(O9Constants.O9_GENERAL_CULTURE);
            ...
            culInfo.DateTimeFormat.ShortDatePattern = GlobalVariable.O9_GLOBAL_HEADOFFICE_PARAM.GENERAL_SHORT_DATE_FMT;
            culInfo.DateTimeFormat.LongDatePattern = GlobalVariable.O9_GLOBAL_HEADOFFICE_PARAM.GENERAL_LONG_DATE_FMT;
            culInfo.DateTimeFormat.ShortTimePattern = GlobalVariable.O9_GLOBAL_HEADOFFICE_PARAM.GENERAL_SHORT_TIME_FMT;
            culInfo.DateTimeFormat.LongTimePattern = GlobalVariable.O9_GLOBAL_HEADOFFICE_PARAM.GENERAL_LONG_TIME_FMT;
    ...
    {
        if (String.IsNullOrEmpty(fmt)) fmt = O9Constants.O9_CONSTANT_SHORT_DATE_FORMAT;
        if (dt != null)
        {
    ...
    {
        if (string.IsNullOrEmpty(stringDate)) return 0;
    ...
    {
        if (string.IsNullOrEmpty(fmt)) fmt = GlobalVariable.O9_GLOBAL_HEADOFFICE_PARAM.GENERAL_DATE_TIME_FMT;
    ...
    {
        return Convert.ToDateTime(dt, new CultureInfo(O9Constants.O9_GENERAL_CULTURE).DateTimeFormat);
    }
    ...
    public static string ConvertStringDatimeToDefaultFormat(string strDatime)
    {
        ...
        {
            DateTime dt;
            if (DateTime.TryParseExact(strDatime, item, new CultureInfo(O9Constants.O9_GENERAL_CULTURE),
    ...
    public static bool IsDatetime(string strDatetime, out string format)
    {
        ...
        {
            if (DateTime.TryParseExact(strDatetime, item, new CultureInfo(O9Constants.O9_GENERAL_CULTURE),
                    DateTimeStyles.None, out dt))
            {
                format = item;
                return true;
    ...
    {
        if (!DateTime.TryParseExact(value.ToString(), O9Constants.O9_CONSTANT_SHORT_DATE_FORMAT,
                new CultureInfo(O9Constants.O9_GENERAL_CULTURE), DateTimeStyles.None, out dt))
        {
    ...
    public static bool CheckValueIsDateExtension(Object value, ref DateTime dt)
    {
        foreach (var item in GlobalVariable.DatetimeFormat)
        {
            if (DateTime.TryParseExact(value.ToString(), item, new CultureInfo(O9Constants.O9_GENERAL_CULTURE),
                    DateTimeStyles.None, out dt))
            {
                return true;
            }
    ...
    {
        if (String.IsNullOrEmpty(fmt)) fmt = GlobalVariable.O9_GLOBAL_HEADOFFICE_PARAM.GENERAL_SHORT_TIME_FMT;
        try
        {
            return String.Format(dt.ToString(), fmt);
        }
        catch (Exception)
        {
            return String.Empty;
    ...
    {
        if (!DateTime.TryParseExact(value.ToString(), O9Constants.O9_CONSTANT_TIME_FORMAT,
                new CultureInfo(O9Constants.O9_GENERAL_CULTURE), DateTimeStyles.None, out dt))
        {
            dt = ConvertToDateTimeFormat(O9Constants.O9_CONSTANT_DATE_COMPARE);
            return false;
    ...
    public static string GenQuery(string query, JObject jsObj)
    {
        try
        {
            ...
            {
                query = query.Replace(O9Constants.O9_CONSTANT_PARAM_KEY + O9Constants.O9_CONSTANT_O9MSG,
                    jsObj.ToString());
            }
                ...
                query.Contains("'" + O9Constants.O9_CONSTANT_TABLE_KEY))
            {
                ...
                {
                    key = "'" + O9Constants.O9_CONSTANT_PARAM_KEY + jsToken.Key.Trim().ToUpper() + "'";
                    tableKey = "'" + O9Constants.O9_CONSTANT_TABLE_KEY + jsToken.Key.Trim().ToUpper() + "'";
                    ...
                    if (query.Contains(key) || query.Contains(tableKey))
                    {
                        if (!String.IsNullOrEmpty(value))
                        {
                            ...
                            if (query.Contains(tableKey)) query = query.Replace(tableKey, value.Replace("'", ""));
                            if (!query.Contains("'" + O9Constants.O9_CONSTANT_PARAM_KEY) &&
                                !query.Contains("'" + O9Constants.O9_CONSTANT_TABLE_KEY)) break;
                        }
                    }
                }
    ...
    public static void SortedFieldsData(SearchFunc sf, ref JArray jarray)
    {
        ...
        string[] lstcolumnNames = dtb.Columns.Cast<DataColumn>()
    ...
    /// 
    /// </summary>
    public static void MergeJsonObject(ref JObject jsObjSrc, JObject jsObjChild, bool isReplace = false,
    ...
    /// 
    /// </summary>
    public static EnmResultResponse AnalysisFunctionResult(string strResponse, out JsonFunction clsJsonFunction)
    {
        try
        {
            ...
            if (clsJsonResponse != null)
            {
                ...
                {
                    JsonFunctionMapping jsonFunctionMapping =
                        JsonConvert.DeserializeObject<JsonFunctionMapping>(clsJsonResponse.GetMessage(),
                            JsonSerializerSetting);
        ...
        {
            System.Console.WriteLine(ex.StackTrace);
            throw new Exception("AnalysisFunctionResult");
        }
    }
    ...
    {
        JArray jsArray = null;
        string key = GlobalVariable.O9_GLOBAL_COMCODE + "." + GlobalVariable.O9_GLOBAL_MEMCACHED_KEY.FoFTab;
    ...
    public static string GenJsonDataByIdRequest(JsonGetDataById clsJsonDataById, string functionId)
    {
        try
        {
            if (clsJsonDataById == null) return "";
            string strResult = "";
            strResult = GenJsonBodyRequest(clsJsonDataById, functionId);
            if (!string.IsNullOrEmpty(strResult))
            {
                if (clsJsonDataById.M) return JsonData.ConvertMappingToOriginal(strResult).ToString();
            }
    ...
    /// <returns></returns>
    public static T JSONDeserializeObject<T>(string str, EnmCacheAction isClientCaching = EnmCacheAction.NoCached)
    {
        try
        {
    ...
    /// <returns></returns>
    public static IPagedList<TResponse> SimpleSearchCommon<TResponse>(SimpleSearchModel model, string sFunc)
    {
    ...
    /// <param name="sFunc"></param>
    /// <returns></returns>
    public static IPagedList<TResponse> AdvanceSearchCommon<TRequest, TResponse>(TRequest model, string sFunc)
    ...
    /// <returns></returns>
    /// <exception cref="Exception"></exception>
    public static JToken RuleFunc(string searchFunc, string currentModules,
        IEnumerable<KeyValuePair<string,object>> condition = null)
    {
        ...
        var strResult = O9Utils.GenJsonBodyRequest(clsJsonSearch, "UTIL_GEN_SEARCH_ADVANCED", "");
        if (!string.IsNullOrEmpty(strResult))
        {
    ...
    public static string MergeQuery(string query, string[] columns, string condition)
    {
        ...
        {
            conAdd += " UPPER(" + columns[i] + ") LIKE '%" + condition.ToUpper() + "%' " + O9Constants.O9_CONSTANT_OR;
            if (i == 1)
            {
                break;
            }
        ...
        if (!string.IsNullOrEmpty(conAdd))
        {
            ...
            if (wherePosition > 0)
            {
                currentQuery = query.Substring(0, wherePosition + 5) + " " + conAdd + " " +
            ...
            else
            {
                ...
                {
                    currentQuery = query.Substring(0, orderByPosition) + "WHERE " + conAdd + " " +
    ...
    public static string GenPassword()
    {
        ...
        for (var i = 0; i < GlobalVariable.O9_GLOBAL_USER_PARAM.GENERAL_PASS_LENGTH; i++)
        {
    ...
    /// <returns></returns>
    /// <exception cref="Exception"></exception>
    public static JsonFrontOffice PostFrontOffice(UserSessions userSessions,string txcode, dynamic data)
    {
        try
        {
    ...
    /// <returns></returns>
    public static List<ButtonInfo> GetCommandRight(string userId, string commandId)
    {
        JObject request = new()
        {
            { "I", commandId },
        ...
        string function = "UTIL_GET_CMDID";
        string strResult = GenJsonBodyRequest(request, function, "", EnmCacheAction.NoCached, EnmSendTypeOption.Synchronize, userId, MsgPriority.Normal);
        List<ButtonInfo> resposne = JsonConvert.DeserializeObject<List<ButtonInfo>>(strResult);
        return resposne;
    }
}
...
public static class JObjectExtensions
{
    ...
    /// <returns></returns>
    public static JObject ConvertListDateStringToLong(this JObject jObject, List<string> list = null)
    {
        try
        {
            ...
            if (list != null)
            {
                ...
                foreach (var item in jObject)
                {
                    ...
                    {
                        jObject[item.Key] = O9Utils.ConvertDateStringToLong(jObject.Value<DateTime>(item.Key)
                            .ToString(O9Constants.O9_CONSTANT_SHORT_DATE_FORMAT + " " +
                                      O9Constants.O9_CONSTANT_TIME_FORMAT));
                    }

                    if (item.Value.Type == JTokenType.Array)
                    {
                        JArray vl = new JArray();
                        ...
                        if (list.Contains(item.Key.ToUpper().Trim()))
                        {
                            if (O9Utils.CheckValueIsDateExtension(item.Value, ref dt))
                            {
                                jObject[item.Key] = O9Utils.ConvertDateStringToLong(
                                    dt.ToString(O9Constants.O9_CONSTANT_SHORT_DATE_FORMAT + " " +
                                                O9Constants.O9_CONSTANT_TIME_FORMAT));
                            }
                        }
                    }
            ...
            else
            {
                foreach (var item in jObject)
                {
                    ...
                    if (item.Value.Type == JTokenType.String)
                    {
                        if (O9Utils.CheckValueIsDateExtension(item.Value, ref dt))
                        {
                            jObject[item.Key] = O9Utils.ConvertDateStringToLong(dt.ToString(
                                O9Constants.O9_CONSTANT_SHORT_DATE_FORMAT + " " + O9Constants.O9_CONSTANT_TIME_FORMAT));
                        }
                    }
                }
            }
    ...
    /// </summary>
    public static Boolean JsonContains(this JObject jsObject, string key)
    {
        Object jsItem = null;
