...
public class SearchFunc {
    ...
    public List<JsonSearchFtag> SearchFtagAnd { 
        get  {
            var respone = new List<JsonSearchFtag>();

            try
            {   
                var query = (from d in this.LSTSFT
                            where (!string.IsNullOrEmpty(d.ORAND) ? d.ORAND.Equals(O9Constants.O9_CONSTANT_AND) : true)
                            && d.INCDT
                            select d).ToList();
                return query;
            }
    ...
    public List<JsonSearchFtag> SearchFtagOr { 
        get {
            ...
            {
                var query = (from d in this.LSTSFT
                             where (!string.IsNullOrEmpty(d.ORAND) ? d.ORAND.Equals(O9Constants.O9_CONSTANT_OR) : true)
                             && d.INCDT
                             select d).ToList();
                return query;
            }
    ...
    /// </summary>
    public string GenSearchCommonSql(Object condition, string strOrAnd = "", EnmOrderTime orderTime = EnmOrderTime.InQuery, bool isSetConditionVisibleField = true, bool hasDistinct = false) {
        try
        {   
            var cdt = "";
            ...
            if (condition != null)
            {
                 ...
                 conditionJObject = condition.GetType().Name == "WhereEnumerableIterator`1" ? (IEnumerable<KeyValuePair<string, object>>) condition : null;
            }
            ...
            if (condition != null && !String.IsNullOrEmpty(cdt))
            {
                ...
                if (this.LSTSFT != null)
                {
                    ...
                    {
                        cdt = cdt1;
                        if (!String.IsNullOrEmpty(sf.FORMAT) || sf.FTAG.Equals("ACNO") || sf.FTAG.Equals("MBACNO") || sf.FTAG.Equals("LBACNO") || "CD".Equals(sf.FTAG.Substring(sf.FTAG.Length - 2)))
                        {
                            if (conditionJObject != null)
                            {
                                var value = conditionJObject.FirstOrDefault(e => e.Key.ToUpper() == sf.FTAG);
                                if (value.Value != null) sf.FVAL = value.Value.ToString();
                            }
                            else
                            {
                        ...
                        if (!String.IsNullOrEmpty(sf.FDEFAULT))
                        {
                            ...
                            if (sf.FVAL == null) sf.FVAL = cdt;
                        }
                        else
                        {       
                            if (sf.FVAL == null && conditionJObject == null) sf.FVAL = cdt;
        ...
        catch (Exception ex)
        {
            ...
            throw new NeptuneException("Error when generate search common sql");
        }
    }
    ...
    /// </summary>
    public string MergeSqlString(string strSelect, string strWhere, string strOrder, bool hasDistinct = false) { 
        try { 
            strSelect = strSelect.Trim(); 
            strWhere = strWhere.Trim(); 
            strOrder = strOrder.Trim(); 

            if (strSelect.EndsWith(O9Constants.O9_CONSTANT_COMMA)) strSelect = strSelect.Substring(0, strSelect.Length - O9Constants.O9_CONSTANT_COMMA.Length); 
            if (strWhere.EndsWith(O9Constants.O9_CONSTANT_OR)) strWhere = strWhere.Substring(0, strWhere.Length - 2); 
            if (strWhere.EndsWith(O9Constants.O9_CONSTANT_AND)) strWhere = strWhere.Substring(0, strWhere.Length - 3); 
            if (strOrder.EndsWith(O9Constants.O9_CONSTANT_COMMA)) strOrder = strOrder.Substring(0, strOrder.Length - O9Constants.O9_CONSTANT_COMMA.Length); 

            strSelect = "SELECT " + (hasDistinct ? "DISTINCT " : "") + strSelect + " FROM " + this.STORV + " A"; 
    ...
    /// Description: GenSearchCommonSql
    /// </summary>
    public string GenSearchCommonSql(string strOrAnd = "", EnmOrderTime orderTime = EnmOrderTime.InQuery, string strWhere = "", bool hasDistinct = false) { 
        try { 
            ...
            foreach (var sf in this.LSTSFT) { 
                ...
                // Gen order by
                if (orderTime == EnmOrderTime.InQuery) strOrder = sf.GenOrderString(strOrder); 
            }

            if (String.IsNullOrEmpty(strWhere)) {

                lstSearchFtagAnd = this.LSTSFT.Where(x => x.INCDT && x.ORAND.Equals(O9Constants.O9_CONSTANT_AND)).ToList();
                lstSearchFtagOr = this.LSTSFT.Where(x => x.INCDT && !x.ORAND.Equals(O9Constants.O9_CONSTANT_AND)).ToList();
                ...
                strWhereOtherAnd = GenWhereString(lstSearchFtagAnd, strWhereOtherAnd, strOrAnd, true); 
                strWhereOtherAnd = ProcessSqlWhereString(strWhereOtherAnd, O9Constants.O9_CONSTANT_AND);
                
                // Gen where string or with has fdefault
                strWhereOtherOr = GenWhereString(lstSearchFtagOr, strWhereOtherOr, strOrAnd, true); 
                strWhereOtherOr = ProcessSqlWhereString(strWhereOtherOr, O9Constants.O9_CONSTANT_AND); 
...
//             CustomizeSearchCommand(sfc, strSelect, strWhere, strOrder, strOrAnd)    'Added by ChiNM
    ...
    /// </summary>
    private string ProcessSqlWhereString(string strWhere, string strOrAnd) { 
        strWhere = strWhere.Trim(); 
        if (!String.IsNullOrEmpty(strWhere)) { 
            if (strWhere.EndsWith(O9Constants.O9_CONSTANT_AND) || strWhere.EndsWith(O9Constants.O9_CONSTANT_OR)) strWhere = strWhere.Substring(0, strWhere.Length - strOrAnd.Length);
    ...
    /// </summary>
    private string GenWhereString(List<JsonSearchFtag> lstSearchFtag, string strWhere, string strOrAnd, bool hasFDefault = false) { 
        if (lstSearchFtag == null) return strWhere;

        foreach (var sf in lstSearchFtag.Where(x => hasFDefault ? !String.IsNullOrEmpty(x.FDEFAULT) : String.IsNullOrEmpty(x.FDEFAULT)).ToList()) { 
            strWhere = GenWhereString(sf, strWhere, strOrAnd); 
            if (String.IsNullOrEmpty(sf.FDEFAULT)) sf.ORAND = ""; 
        }
    ...
    /// Description: GenWhereString
    /// </summary>
    private string GenWhereString(JsonSearchFtag sf, string strWhere, string strOrAnd = "") { 
        try { 
            string strIn = ""; 
            ...
            if (sf.FVAL != null && sf.INCDT) { 
                ...
                switch (sf.FTYPE.ToUpper()) { 
                    ...
                    case "J": 
                    case "V": 
                        if (!(sf.FVAL is DBNull) && !(sf.FVAL is List<String>)) str = sf.FVAL.ToString();
                        ftag = sf.FWHERE;

                        if (!str.ToUpper().StartsWith("IS NULL") && !str.ToUpper().StartsWith("IS NOT NULL")) { 
                            if (!(sf.FVAL is List<String>)) { 
                                if (!String.IsNullOrEmpty(sf.OPRT) && sf.OPRT.ToUpper().Equals("CONTAINS")) { 
                                    strWhere = String.IsNullOrEmpty(str) ? strWhere : strWhere + sf.OPRT + "(" + ftag + ",'" + str + "')>0" + orAnd; 
                                } else if (!String.IsNullOrEmpty(sf.OPRT) || !String.IsNullOrEmpty(sf.OPRTVAL)) { 
                                    strWhere = String.IsNullOrEmpty(str) ? strWhere : strWhere + "UPPER(" + ftag + ") =" + (String.IsNullOrEmpty(sf.OPRT) ? sf.OPRT : sf.OPRTVAL) + " '" + str.ToUpper() + "'" + orAnd; 
                                } else { 
                                    strWhere = String.IsNullOrEmpty(str) ? strWhere : strWhere + "UPPER(" + ftag + ") LIKE '%" + str.ToUpper() + "%'" + orAnd; 
                                }
                            } else { 
                                if (((List<String>)sf.FVAL).Count > 0) { 
                                    ...
                                    if (strIn.EndsWith(O9Constants.O9_CONSTANT_COMMA)) strIn = strIn.Substring(0, strIn.Length - O9Constants.O9_CONSTANT_COMMA.Length); 
                    ...
                    case "N":
                    case "B": 
                        if (!(sf.FVAL is DBNull) && !(sf.FVAL is List<String>)) { 
                            if (!(Double.TryParse(sf.FVAL.ToString(), out dbl))) dbl = Double.MinValue; 
                            strWhere = dbl == Double.MinValue ? strWhere : strWhere + sf.FTAG + (String.IsNullOrEmpty(sf.OPRTVAL) ? "=" : sf.OPRTVAL) + dbl + orAnd; 
                        } else { 
                            if (sf.FVAL is List<String>) { 
                                foreach (var s in (List<String>)sf.FVAL) { 
                                    strIn += s.ToUpper() + O9Constants.O9_CONSTANT_COMMA; 
                        ...
                        break;
                    case "D": 
                        if (!(sf.FVAL is DBNull)) { 
                            if (O9Utils.CheckValueIsDate(sf.FVAL, ref dt)) { 
                                strWhere = dt != O9Utils.ConvertToDateTimeFormat(O9Constants.O9_CONSTANT_DATE_COMPARE) ? strWhere + "TO_DATE(" + sf.FTAG + ",'" + O9Constants.O9_CONSTANT_DATE_FORMAT + "') " + (sf.OPRTVAL != null && !(String.IsNullOrEmpty(sf.OPRTVAL)) ? sf.OPRTVAL : "=") + " TO_DATE('" + O9Utils.ConvertDateToStringFormat(dt) + "','" + O9Constants.O9_CONSTANT_DATE_FORMAT + "')" + orAnd : strWhere;
                            } else if (O9Utils.CheckValueIsTime(sf.FVAL, ref dt)) { 
                                strWhere = dt != O9Utils.ConvertToDateTimeFormat(O9Constants.O9_CONSTANT_DATE_COMPARE) ? strWhere + sf.FTAG + (sf.OPRTVAL != null &&  !(String.IsNullOrEmpty(sf.OPRTVAL)) ? sf.OPRTVAL : "=") + "'" + O9Utils.ConvertTimeToStringFormat(dt, O9Constants.O9_CONSTANT_TIME_FORMAT) + "'" + orAnd : strWhere;
                            }
                        }
                        break; 
                    default: 
            ...
            if (data is JArray) { 
                foreach (var row in data.Children<JObject>()) { 
                    if (this.LSTSFT.Count > 0) {
                        ...
                        {   
                            if (row.ContainsKey(item2.FTAG.Replace("_CDID","").ToLower()) && row.ContainsKey(item2.FTAG.ToLower()))
                            {
                                row[item2.FTAG.Replace("_CDID", "").ToLower()] = row[item2.FTAG.ToLower()];
