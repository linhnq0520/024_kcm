...
/// </summary>
public partial class ApiNcbsApiTag : ICoreAPIService
{
    /// <summary>
    /// 
    /// </summary>
    ...
    public async Task<JToken> BuildResponse(JToken packApi)
    {
        ...
        var result = packApi;
        await Task.CompletedTask;
        if (result.SelectToken("data") != null)
        {
            JArray data = new JArray();
            foreach (var item in result.SelectToken("data").ToJArray())
            {
                JObject new_item = new JObject();
                new_item["code_id"] = item;
                new_item["caption"] = item;
                data.Add(new_item);
            }
            result["data"] = data;
        }
    ...
    public async Task<JToken> BuildResponse(ExecuteResponseModel packApi)
    {
        ...
        {
            JArray data = new JArray();
            foreach (var item in result.Data.ToJArray())
            {
                JObject new_item = new JObject();
                new_item["code_id"] = item;
                new_item["caption"] = item;
