...
/// </summary>
public partial class ApiSystemEvent : ICoreAPIService
{
    /// <summary>
    /// 
    /// </summary>
    ...
    {
        if (packApi["apply_specific_workflows"] != null)
        {
            var listWorkflow = new List<object>();
            foreach (var item in packApi["apply_specific_workflows"].ToJArray())
            {
                var item_wf = new JObject();
                item_wf["apply_specific_workflows"] = new
                {
                    wf_id = item.ToString()
                }.ToJObject();

                listWorkflow.Add(item_wf);
            }
            packApi["apply_specific_workflows"] = listWorkflow.ToJArray();
        }
        await Task.CompletedTask;
        return packApi;
    }
