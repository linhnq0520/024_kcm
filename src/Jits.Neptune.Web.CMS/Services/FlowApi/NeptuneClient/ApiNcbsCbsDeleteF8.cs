...
/// </summary>
public partial class ApiNcbsCbsDeleteF8 : ICoreAPIService
{
    /// <summary>
    /// 
    /// </summary>
    ...
    {
        var workflow_ = JObject.Parse(packApi);
        if (workflow_ != null)
        {

            if (workflow_["request_body"] != null)
            {
                var requestBody = workflow_["request_body"].ToJObject();

                if (requestBody != null)
                {
                    var workflowInput = requestBody.ToObject<WorkflowExecuteModel>();

                    if (workflowInput != null)
                    {
                        workflowInput.fields["delete_username"] = EngineContext.Current.Resolve<JWebUIObjectContextModel>().Bo.GetBoInput().SelectToken("user.username").ToString();
                        workflowInput.workflow_type = 1;
                        workflowInput.reversal_execution_id = workflow_["ref_id"].ToString();
