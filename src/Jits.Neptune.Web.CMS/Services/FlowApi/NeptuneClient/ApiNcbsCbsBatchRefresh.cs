...
/// </summary>
public partial class ApiNcbsCbsBatchRefresh : ICoreAPIService
{
    /// <summary>
    /// 
    /// </summary>
    ...
    public async Task<JToken> BuildResponse(JToken packApi)
    {
        ...
        var data_res = JToken.Parse(content.response.data.ToString());
        int dataCount = 0;
        bool isFailed = false;
        BatchSumnaryModel O9_job_process_summary = new BatchSumnaryModel();
        if (Utils.Utils.IsValidJsonArray(data_res.ToSerialize()))
        {
            foreach (var itemStep in data_res.ToJArray())
            {
        ...
        var current = Math.Round((double)dataCount / data_res.ToJArray().Count * 100);

        O9_job_process_summary.Current = current.ToString();
        O9_job_process_summary.IsFailed = isFailed;
        JObject result_ = new JObject();
        result_["O9_job_process_refesh_table_summary"] = data_res;
