...
/// </summary>
public partial class ApiNcbsCbsFo : ICoreAPIService
{
    /// <summary>
    /// 
    /// </summary>
        ...
        try
        {
            ...
            if (responseWorkflow.request.request_header.tx_context != null)
            {
                if (responseWorkflow.request.request_header.tx_context["transaction_number"] != null)
                    if (!string.IsNullOrEmpty(responseWorkflow.request.request_header.tx_context["transaction_number"].ToString()))
                ...
                for (var index = 0; index < vouchers.Count; index++)
                {
                    ...
                    if (voucher_code == "A4")
                    {
                        if (tags.SelectToken("transaction_body.deposit_sub_type") != null)
                        {
                            ...
                            var deposit_sub_type = tags.SelectToken("transaction_body.deposit_sub_type").ToString();
    ...
    public async Task<List<ErrorInfoModel>> BuildError(WorkflowExecutionInquiry responseApiModel)
    {
        ...
        EngineContext.Current.Resolve<JWebUIObjectContextModel>().Bo.GetBoInput()["fo_execution_id"] = responseApiModel.execution.execution_id;
        if (!string.IsNullOrEmpty(responseApiModel.execution.approved_execution_id))
            EngineContext.Current.Resolve<JWebUIObjectContextModel>().Bo.GetBoInput()["fo_execution_id"] = responseApiModel.execution.approved_execution_id;
    ...
    public async Task<bool> CheckHasError(ExecuteResponseModel packApi)
    {
        ...
        else if (packApi.Status.Equals("ERROR")) return true;
            ...
            if (boInput["static_token"] != null)
            {
                ...
                if (boInput["JWEBUI_O9_approve"] != null)
                    if (boInput["JWEBUI_O9_approve"]["fo_ref_id"] != null)
                        workflowExecute.approved_execution_id = boInput["JWEBUI_O9_approve"]["fo_ref_id"].ToString();
            }
