...
public class TxNcbsLicenseLogin
{
    /// <summary>
    ///context
    /// </summary>
    ...
    public TxNcbsLicenseLogin(IFoService foService, IAppService appService, ILangService langService, IParaServerService paraServerService, IAdminServices adminServices, IAdminGrpcService adminGrpcService, IAuthenticationGrpcService authenticationGrpcService)
    {
    ...
    public async Task<string> loginApi(string learn_api)
    {
        ...
        if (!infoUserLogin.UserId.ToString().Equals(""))
        {
            ...
            {
                var getUserInfo = await _adminService.GetUserAccountById(infoUserLogin.UserId.ToString());
                if (getUserInfo != null)
                {
