...
IF OBJECT_ID('[B_CRD_RPPD_EXEC]', 'P') IS NOT NULL
    BEGIN
    -- The procedure exists
    DROP PROCEDURE B_CRD_RPPD_EXEC;
END;
    GO

CREATE PROCEDURE [dbo].B_CRD_RPPD_EXEC
    (
...
BEGIN TRY
	...
	From (select * from dbo.CreditAccount where Crdsts in (0, 2, 3) and CreditFacility != 'OD') acc 
				...
				inner join (select * from dbo.CreditIFCDefinition where IfcSubType = 'PP') ifcDef on ifcbalance.IfcCode =  ifcDef.IfcCode
	...
	FROM (
		select
		'CRD' as [ModuleCode]
		, list.CreditAccountNumber as [AccountNumber]
    ...
    ([AccountNumber], [TransactionDate], [TransactionRefId], [ValueDate], [TransactionCode], [Amount], [Dorc], [Description], [UserCreated], [STATUS], [UserApprove], [TransactionNumber], [ChannelId])
	SELECT AccountNumber, --DefAccountNumber
		...
		Amount, --Amount
		'C', --Dorc
		@StepName + ' Penalty principal collection. A/c: [' + AccountNumber + ']', -- [Description] --Description
	...
	SELECT
		NewID() as [TransId]
			...
			, 'DEBIT_PENALTY_PRIN' as [TransCode] 
	...
	SELECT
		NewID() as [TransId]
			...
			, list.CreditAccountNumber as [CreditAccount]
			, ifcbaldetail.[IFCCode] as [IFCCode]
			, 'CREDIT_PAID_PENALTY_PRIN' as [TransCode] 
			, list.CreditClassificationStatus as  [CurrentClassificationStatus]
			, 'N' as  [TransactionStatus]
	 ...
	 Insert into o9deposit.dbo.DepositStatement ([AccountNumber], [StatementDate], [ReferenceId], [ValueDate], [Amount], [CurrencyCode], [ConvertAmount], [StatementCode], [StatementStatus], [RefNumber], [TransCode], [TransactionNumber], [Description], [CreatedOnUtc], [UpdatedOnUtc])
		Select
           acc.AccountNumber 
			...
			, @TransactionNumber
			, @StepName + ' Penalty principal collection. A/c: [' + CreditAccountNumber + ']'-- [Description]
			, GETUTCDATE()
			, GETUTCDATE()
